# Hi, I'm Ritika Anand 

### ECE @ VIT Chennai | C/C++ | Python | Embedded Systems | AI/ML | RTL

Final-year Electronics and Communication Engineering student at VIT Chennai with hands-on experience across **software development, embedded systems, deep learning, digital design, and system-level engineering**.

I'm interested in building reliable software and hardware systems and solving real-world problems through technology.

---

## 🛠️ Technical Skills

### Programming
- C
- C++
- Python
- Embedded C
- MATLAB
- Assembly

### Software & Computer Science
- Data Structures & Algorithms
- Object-Oriented Programming
- Machine Learning
- Deep Learning
- Computer Vision
- Git & GitHub

### Embedded Systems
- 8051
- ESP32
- Arduino
- Raspberry Pi
- Microcontrollers
- Sensors & Actuators
- Embedded Firmware
- Hardware Debugging

### Digital Design & RTL
- Verilog
- VHDL
- RTL Design
- Digital Electronics
- ModelSim

### AI / ML
- PyTorch
- TensorFlow
- Keras
- NumPy
- Pandas
- Scikit-learn
- CNN
- Transfer Learning

### Engineering Tools
- MATLAB
- Keil
- Proteus
- LTspice
- Silvaco Atlas
- Ansys HFSS
- Cadence Virtuoso

---

## Featured Projects

###  Alzheimer's Disease Classification using MRI

Deep learning project for classifying MRI scans into **Normal, Mild Cognitive Impairment (MCI), and Alzheimer's Disease (AD)**.

Implemented and compared:

- 2D CNN
- 3D CNN
- Hybrid 2D + 3D CNN
- ResNet18-based feature extraction
- MRI/DICOM preprocessing
- Model evaluation

**Tech:** Python, PyTorch, TensorFlow, Keras, CNN, ResNet18, PyDICOM

🔗 [View Project](https://github.com/Ritika2718/alzheimers-mri-deep-learning)

---

### SmartRover – Dual-Control Obstacle Avoiding Robot

Built a dual-control robotic system using **MPU6050 gesture control** and **ESP8266-based mobile control**, with sensor-based obstacle detection.

**Tech:** Embedded C, ESP8266, MPU6050, IR Sensors, Ultrasonic Sensors, Arduino

🔗 [View Project](https://github.com/Ritika2718/SmartRover)

---

###  Home Automation using 8051

Developed an occupancy-based home automation system using the **AT89C51 microcontroller**, sensors and automated lighting control.

**Tech:** Embedded C, 8051, Sensors, Keil, Proteus

---

###  Wireless Data Transmission using Li-Fi

Developed a Li-Fi prototype for wireless data transmission using LED-based optical communication and evaluated link reliability under different channel conditions.

**Tech:** Embedded Systems, Optical Communication, MATLAB, Electronics

---


---

##  Currently Strengthening

- Data Structures & Algorithms
- C / C++
- Python
- Operating Systems
- Computer Architecture
- Embedded Systems
- Digital Design & RTL
- Machine Learning
- System Design Fundamentals

---

##  Certifications

- C/C++/Python – IIT Bombay
- Microsoft Azure AI Fundamentals
- Generative AI – Microsoft/LinkedIn
- Data Visualization – Tata Group
- Threat Modeling – Infosys
- Software Engineering Virtual Internship – Goldman Sachs

---


---

##  Coding & Problem Solving

### LeetCode

I regularly practice **Data Structures & Algorithms, problem solving and coding challenges**.

🔗 [LeetCode Profile](https://leetcode.com/u/ritika2718/)

---

##  Let's Connect

-  [GitHub](https://github.com/Ritika2718)
-  [Portfolio](https://ritikawork.vercel.app)
-  [LeetCode](https://leetcode.com/u/ritika2718/)

---

### 💡 Open to opportunities in

**Software Engineering | Embedded Systems | Firmware | AI/ML | Computer Vision | RTL/Digital Design | Systems Engineering**

---

⭐ Thanks for visiting my profile!
